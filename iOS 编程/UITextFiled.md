...
如果设置```为UITextBorderStyleRoundedRect```，自定义背景图像被忽略。   
*样式类型：*
...
### 换行键自动监测功能
如果将该属性设置为YES，*UITextField* 对象会自动监测用户输入，并根据是否输入了文字启用/禁用换行键,如果文本内容长度为0，则禁用换行键，如果文本内容长度不为0，则启用换行键.
...
* UIReturnKeyContinue NS_ENUM_AVAILABLE_IOS(9_0),   标有Continue的蓝色按钮
...
### 按return键盘放弃第一响应者
UITextfield实例要遵守```UITextFieldDelegate```协议：
...
* <http://blog.csdn.net/u010850094/article/details/51525964>
* [如何设计出更好的表单？](http://www.jianshu.com/p/b3ad55a55448)
### 未完待续...
